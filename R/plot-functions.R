...
# Plot densities of logLRs. Uses ggh4x for facetting
plotDensities = function(logLRs, cut = 0, adj = 1.5, xlim = NULL, ylim = NULL,
                         file = NULL) {

  plotdat = calcD(logLRs) |>
    mutate(Model = fct_recode(Model, "‘equal’" = "Equal", "‘stepwise’" = "Stepwise"))
  ...
  g = ggplot(plotdat, aes(x = D, fill = Transform)) +
    ...
    labs(x = expression(log[2]~(LR[M]/LR[R])), y = NULL) +
...
# Alternative version of plotDensities; sets panel limits explicitly
